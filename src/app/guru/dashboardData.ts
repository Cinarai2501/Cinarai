...
export function buildCompletionStatusSummary(
  ...
  const statuses = [
    { label: 'Belum memulai', description: 'Belum ada progress', matcher: (documents: ComicProgressDocument[]) => !documents.length || documents.every((document) => (document.percentage ?? 0) === 0) },
    { label: 'Sedang belajar', description: 'Sudah ada progress tetapi belum selesai', matcher: (documents: ComicProgressDocument[]) => documents.some((document) => (document.percentage ?? 0) > 0 && (document.percentage ?? 0) < 100 && document.status !== 'completed') },
    { label: 'Sudah selesai', description: 'Minimal satu komik selesai', matcher: (documents: ComicProgressDocument[]) => documents.some((document) => document.status === 'completed' || (document.percentage ?? 0) >= 100) },
  ];

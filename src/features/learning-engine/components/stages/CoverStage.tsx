...
export default function CoverStage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Cover Image */}
      <div className="relative w-full aspect-[3/4] rounded-2xl overflow-hidden shadow-md bg-neutral-200">
        <Image
          src={comic.cover}
      ...
      {/* Judul & Subjudul */}
      <div>
        <h2 className="text-xl font-bold text-neutral-950 sm:text-2xl leading-snug">
          {comic.title}
        </h2>
        <p className="mt-1 text-sm text-neutral-500">{comic.subtitle}</p>
      </div>
      ...
      <section>
        ...
        <ul className="flex flex-col gap-2">
          {comic.learningTargets.map((target, i) => (
            <li key={i} className="flex items-start gap-3 rounded-xl bg-white p-3 shadow-xs">

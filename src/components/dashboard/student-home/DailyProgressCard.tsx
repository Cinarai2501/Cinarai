...
export default function DailyProgressCard({ progressPct, stageLabel }: DailyProgressCardProps) {
  return (
    <section className="rounded-[28px] bg-[#F6F9FE] p-[18px] shadow-[0_10px_24px_rgba(15,23,42,0.08)]" style={{ minHeight: 162 }}>
      <div className="flex items-start justify-between gap-[8px]">
        ...
        <Link href="/dashboard/siswa/komik" className="flex h-[44px] shrink-0 items-center rounded-full bg-[#1D93FF] px-[14px] text-[12px] font-semibold text-white shadow-[0_8px_20px_rgba(29,147,255,0.18)]" aria-label="Lihat detail progres">
          Lihat
        </Link>
      </div>
      ...
      <div className="mt-[10px] flex items-center justify-between gap-[12px] text-[12px] font-semibold text-[#6B7280]">
        <span>Tahap selesai</span>
        <span>{progressPct}%</span>
        <span>Jumlah tahap</span>
      </div>
    </section>
  );
}

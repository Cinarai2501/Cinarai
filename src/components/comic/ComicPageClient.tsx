...
export default function ComicPageClient({ comicId }: ComicPageClientProps) {
  ...
  const loadComic = useCallback(() => {
    ...
    const timeout = setTimeout(() => {
      if (!cancelled) {
        ...
        setFetchError('Koneksi terlalu lama. Periksa internet kamu.');
        setLoading(false);
      }
    }, FETCH_TIMEOUT_MS);
    ...
    fetchComicById(comicId)
      .then((data) => {
        ...
        if (!data) {
          ...
          setFetchError(`Komik ${comicId} tidak ditemukan.`);
        } else {
          setComic(data);
        }
      })
      .catch((error) => {
        ...
        setFetchError('Gagal memuat data komik. Periksa koneksi internet kamu.');
      })
      .finally(() => {
        if (!cancelled) {
  ...
  }, [state.sintaksList, complete, comicId, router]);
  ...
  if (fetchError || !comic) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center bg-gray-900 px-6 gap-4 text-center">
        ...
        <p className="text-base font-black text-white">Gagal Memuat Komik</p>
        <p className="text-sm text-gray-400">{fetchError ?? 'Data komik tidak tersedia.'}</p>
        <button
          onClick={loadComic}
          className="rounded-2xl bg-primary-600 px-6 py-3 text-sm font-black text-white hover:bg-primary-700 active:scale-[0.98] transition-all"
        >
          Coba Lagi
        </button>
        <Link href="/dashboard" className="text-sm text-gray-400 underline underline-offset-2">
